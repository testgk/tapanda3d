...
def _getRelativeHpr( bodyPart: NodePath, target_position: Vec3, tracking_speed = 100) -> Tuple[ int, Vec3 ]:
...
class StationaryMovementManager:

	def __init__( self, entity, world, render ):
		self.__obstacleDetector = ObstacleDetector(entity, world, render)
		self.__tower: Tower | Mover = entity

	@property
	def obstacleDetector( self ) -> ObstacleDetector:
		return self.__obstacleDetector

	def track_target_coreBody_angle( self, task ):
		task.delayTime = 0.5
		if self._getCurrentTarget() is None:
			return task.again
		h_diff, new_hpr = _getRelativeHpr( self.__tower.coreBodyPath,
	...
	def maintain_turret_angle( self, task ):
		if self._getCurrentTarget() is None:
			return task.cont
		h_diff, new_hpr = _getRelativeHpr( self.__tower.turretBase().rigidBodyPath,
	...
	def _getCurrentTarget( self ) -> Target:
